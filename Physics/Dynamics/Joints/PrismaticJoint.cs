...
namespace PE2.Physics.Dynamics
{
	...
	public class PrismaticJoint : Joint
	{
		...
		internal override bool SolvePositionConstraints(float baumgarte)
		{
			...
			Vector2 r1 = Common.Math.Mul(R1, _localAnchor1 - _localCenter1);
			Vector2 r2 = Common.Math.Mul(R2, _localAnchor2 - _localCenter2);
			Vector2 d = c2 + r2 - c1 - r1;

			if (_enableLimit)
			{
				_axis = Common.Math.Mul(R1, _localXAxis1);
			...
			_perp = Common.Math.Mul(R1, _localYAxis1);
			...
			linearError = Common.Math.Max(linearError, Common.Math.Abs(C1.X));
			angularError = Common.Math.Abs(C1.Y);
