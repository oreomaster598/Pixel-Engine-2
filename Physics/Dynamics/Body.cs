...
namespace PE2.Physics.Dynamics
{
	...
	public class Body : IDisposable
	{
		...
		{
			Box2DNetDebug.Assert(world._lock == false);
			...
			_xf.rotation = Common.Math.AngleToRotation(bd.Angle);
		...
		public void Dispose()
		{
			Box2DNetDebug.Assert(_world._lock == false);
		...
		internal bool SynchronizeFixtures()
		{
			...
			xf1.rotation = Common.Math.AngleToRotation(_sweep.A0);
		...
		public void DestroyFixture(Fixture fixture)
		{
			...
			Box2DNetDebug.Assert(fixture.Body == this);

			// Remove the fixture from this body's singly linked list.
			Box2DNetDebug.Assert(_fixtureCount > 0);
		...
		{
			return SetTransform(position, Common.Math.AngleToRotation(angle));
		}
...
#else
			SetTransform(position, Common.Math.AngleToRotation(GetAngle()));
#endif
		}
...
#else
			SetTransform(GetPosition(), Common.Math.AngleToRotation(angle));
#endif
		}
		...
		internal void SynchronizeTransform()
		{
			...
			_xf.rotation = Common.Math.AngleToRotation(_sweep.A);
