						...
						else
						{
							...
							for (; ; )
							{
								...
								if (x.x >= 0.0f && x.y >= 0.0f)
								{
									...
									Box2DNetDebug.Assert(Common.Math.Abs(vn1 - cp1.VelocityBias) < k_errorTol);
									Box2DNetDebug.Assert(Common.Math.Abs(vn2 - cp2.VelocityBias) < k_errorTol);
#endif
									break;
								}
					...
					case ManifoldType.Circles:
						{
							...
							if ((pointA - pointB).LengthSquared() > (Common.Math.Epsilon *Common.Math.Epsilon))
							{
