...
namespace PE2.Physics.Collision
{
	public class Pair
	{
		[Flags]
	...
	public class PairManager
	{
		...
		public PairManager()
		{
			Box2DNetDebug.Assert(Common.Math.IsPowerOfTwo((uint)PairManager.TableCapacity) == true);
			Box2DNetDebug.Assert(PairManager.TableCapacity >= Settings.MaxPairs);
		...
		*/
		...
		public void RemoveBufferedPair(int id1, int id2)
		{
			...
			if (pair.IsBuffered() == false)
			{
				...
				Box2DNetDebug.Assert(pair.IsFinal() == true);
		...
		public void Commit()
		{
			...
			for (int i = 0; i < _pairBufferCount; ++i)
			{
				...
				Box2DNetDebug.Assert(pair.IsBuffered());
				pair.ClearBuffered();

				Box2DNetDebug.Assert(pair.ProxyId1 < Settings.MaxProxies && pair.ProxyId2 < Settings.MaxProxies);

				Proxy proxy1 = proxies[pair.ProxyId1];
				Proxy proxy2 = proxies[pair.ProxyId2];

				Box2DNetDebug.Assert(proxy1.IsValid);
				Box2DNetDebug.Assert(proxy2.IsValid);
				...
				{
					Box2DNetDebug.Assert(_broadPhase.TestOverlap(proxy1, proxy2) == true);
		...
		private Pair Find(int proxyId1, int proxyId2, uint hash)
		{
			...
			Box2DNetDebug.Assert(index < Settings.MaxPairs);
		...
		private Pair AddPair(int proxyId1, int proxyId2)
		{
			...
			Box2DNetDebug.Assert(_pairCount < Settings.MaxPairs && _freePair != PairManager.NullPair);
		...
		{
			Box2DNetDebug.Assert(_pairCount > 0);
		...
		private void ValidateBuffer()
		{
			...
			for (int i = 0; i < _pairBufferCount; ++i)
			{
				if (i > 0)
				{
					Box2DNetDebug.Assert(Equals(_pairBuffer[i], _pairBuffer[i - 1]) == false);
				}

				Pair pair = Find(_pairBuffer[i].ProxyId1, _pairBuffer[i].ProxyId2);
				Box2DNetDebug.Assert(pair.IsBuffered());

				Box2DNetDebug.Assert(pair.ProxyId1 != pair.ProxyId2);
				Box2DNetDebug.Assert(pair.ProxyId1 < Settings.MaxProxies);
				Box2DNetDebug.Assert(pair.ProxyId2 < Settings.MaxProxies);

				Proxy proxy1 = _broadPhase._proxyPool[pair.ProxyId1];
				Proxy proxy2 = _broadPhase._proxyPool[pair.ProxyId2];

				Box2DNetDebug.Assert(proxy1.IsValid == true);
				Box2DNetDebug.Assert(proxy2.IsValid == true);
			}
#endif
		}
		...
		private void ValidateTable()
		{
			...
			for (int i = 0; i < PairManager.TableCapacity; ++i)
			{
				...
				{
					Pair pair = _pairs[index];
					Box2DNetDebug.Assert(pair.IsBuffered() == false);
					Box2DNetDebug.Assert(pair.IsFinal() == true);
					Box2DNetDebug.Assert(pair.IsRemoved() == false);
